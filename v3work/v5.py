...
class FarmState:
    def __init__(self, obs):
        ...
        # --- Opponent imminent-harvest signal (v4 addition) ---
        # If the opponent has a plant/animal that's ALREADY ready to harvest
        # (yield_units > 0) or about to be (within 1 day of its bonus/
        # production window), they're likely to harvest+sell soon, which
        # will crash that item's price. Selling our own matching inventory
        # ahead of that beats waiting into a worse price.
        self.opp_imminent_products = set()
        if 0 <= opp_idx < len(self.farms):
            opp_farm = self.farms[opp_idx]
            for row in opp_farm.get("tiles", []):
                for tile in row:
                    ...
                    if tile.get("kind") == "PLANT" and tile.get("yield_units", 0) > 0:
                        self.opp_imminent_products.add(tile.get("crop"))
                    elif tile.get("kind") in ("COOP", "PASTURE") and tile.get("animal") \
                         and tile.get("yield_units", 0) > 0:
                        self.opp_imminent_products.add(ANIMALS.get(tile["animal"], {}).get("product"))
...
def find_sell_orders(state, days_left):
    ...
    for item, qty in state.shed.items():
        ...
        # v4: opponent has a matching product ready/near-ready to harvest -
        # their sell is coming, likely crashing this price. Get ahead of it
        # rather than waiting for a "better" price that won't materialize.
        opp_dumping_soon = item in state.opp_imminent_products

        if shed_full or opp_dumping_soon:
            floor_mult = 0.5 if shed_full else 0.35
            sell_qty, _ = best_sell_batch(item, qty, inv, state.m_params, 
                                          min_marginal_price=max(PRICE_FLOOR, int(cur_price * floor_mult)))
            if sell_qty > 0:
...
def _best_profit_per_tile_per_day(money, days_left):
    """Real per-tile-per-day profit estimate, reusing the same profit math
    as find_plant_tasks - used to value land expansion honestly instead of
    a magic constant."""
    best = 0
    for crop, cd in CROPS.items():
        seed_cost = cd.get("seed", 999)
        ...
        if cd.get("ongoing", False):
            ...
            max_day = cd.get("max_yield_day", 999)
            expected_yield = max_yield if days_left >= max_day + 1 else max(1, days_left - 1)
        profit = expected_yield * base_price - seed_cost
        days_occ = cd.get("days_occupied", 1)
        per_day = profit / max(1, days_occ)
        if per_day > best:
            best = per_day
    return max(0, best)
...
def find_land_orders(state, money, days_left):
    ...
    # Real expected value, not a magic constant: best-available crop's
    # profit/tile/day, times a new quadrant's 25 tiles, times remaining
    # useful days, discounted for realistic throughput (new land can't be
    # fully worked the instant it's unlocked - workers have to travel to
    # and plant every new tile, which takes real turns).
    per_tile_per_day = _best_profit_per_tile_per_day(money - state.next_land_price, days_left)
    new_tiles = 25
    usable_days = max(0, days_left - 2)  # buffer for travel/setup time
    throughput_discount = 0.4  # can't work all 25 new tiles from turn one
    expected_value = per_tile_per_day * new_tiles * usable_days * throughput_discount

    if expected_value > state.next_land_price * 1.3:  # require real margin, not just >0
